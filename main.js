...
ipcMain.handle('open-or-install-product', async (_e, payload) => {
  try {
    ...
    // Chỉ trả về thông tin sản phẩm, không xử lý download hay cài đặt
    return {
      ok: true,
      product: {
        ...
        expected_publisher: product.expected_publisher || product.publisher || '',
        expected_product_name: product.expected_product_name || product.name || '',
        is_portable: product.is_portable || false
      }
    };
  } catch (err) {
    return { ok: false, message: err.message };
